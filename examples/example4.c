...
static int patestCallback(const void *inputBuffer, uint8_t *outputBuffer,
	unsigned long framesPerBuffer,
	const PaStreamCallbackTimeInfo* timeInfo,
	PaStreamCallbackFlags statusFlags,
	void *userData)
{
	int len = framesPerBuffer * 2 * sizeof(float);
	...
	for (SampleBlock = TSF_RENDER_EFFECTSAMPLEBLOCK; SampleCount; SampleCount -= SampleBlock, outputBuffer += (SampleBlock * (2 * sizeof(float))))
	{
		...
		tsf_render_float(g_TinySoundFont, (float*)outputBuffer, SampleBlock, 0);
	}
	return 0;
}
...
int main(int argc, char *argv[])
{
	...
	if (err != paNoError) 
	{
		fprintf(stderr, "Could not open default stream\n");
		return 1;
	}
	...
	TinyMidiLoader = tml_load_filename("M:\\transfer\\Kar\\favorites\\Kaoma - Chorando se foi.kar"); // "venture.mid");
	if (!TinyMidiLoader)
	{
